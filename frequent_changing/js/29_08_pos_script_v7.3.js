        ...
        return new Promise(function (resolve, reject) {
            ...
            objectStore.openCursor().onsuccess = function (event) {
                ...
                if (cursor) {
                    ...
                    if (type == 2) {
                        sale_id_temp = Number(cursor.value.sale_id);
                    }
                    if (sale_id_temp == Number(sale_id)) {
                        let orderData = cursor.value;
                        let orderInfo = {
                            order: orderData.order,           // The order data you want to return
                            sale_inv_no: orderData.sale_inv_no, // sale_inv_no
                            inv_no: orderData.inv_no           // inv_no
                        };
                        resolve(orderInfo);
                    }
                    cursor.continue();
        ...
        return new Promise(function (resolve, reject) {
            ...
            objectStore.openCursor().onsuccess = function (event) {
                ...
                if (cursor) {
                    if (Number(cursor.value.sales_id) == Number(sale_id)) {
                        let orderData = cursor.value;
                        let orderInfo = JSON.parse(orderData.order);

                        orderInfo.sale_inv_no = orderData.sale_inv_no || "";

                        resolve(JSON.stringify(orderInfo));
                    }
                    cursor.continue();
                }
            };
...
//     };
		...
		objectStore.openCursor().onsuccess = function (event) {
			...
			if (cursor) {
				if (cursor.value.sales_id == sale_id) {
					...
					// ⚠️ Soft delete: only mark, don’t remove until sync confirmed
					updateData.is_invoice = 2;
					let request = cursor.update(updateData);

					request.onsuccess = function () {
						$("#order_" + sale_id).addClass("deleted_order");
						clearFooterCartCalculation();
						displayOrderList();
					};
					return;
	...
	function getNextSaleInvNo(){
        $.ajax({
            url: base_url + "Sale/get_invoiceNumber",
            method: 'GET',
            cache: false,
            success: function (response) {
                const data = JSON.parse(response);
                // Add the response data to order_info
                main_inv_no = data.inv_no;
                main_sale_inv_no = data.sale_inv_no;
                console.log("main_sale_inv_no ", main_sale_inv_no);
            },
            error: function () {
...
//     }
	...
	function add_to_recent_sale_by_ajax(order_object, sale_id, is_ignore = '') {
		...
		let objectStore = transaction.objectStore("recent_sales");
		// Step 1: Count total entries
		let countRequest = objectStore.count();
		countRequest.onsuccess = function () {
			let count = countRequest.result;
			if (count >= 20) {
				// Step 2: Delete oldest entry
				let deleteCursor = objectStore.openCursor();
				deleteCursor.onsuccess = function (event) {
					let cursor = event.target.result;
					if (cursor) {
						cursor.delete(); // delete the oldest record
						insertNewOrder(); // add the new one
					}
				};
			} else {
				insertNewOrder(); // directly add new order if less than 10
			}
		};
		// Step 3: Insert new order
		function insertNewOrder() {
			let addRequest = db.transaction("recent_sales", "readwrite")
			...
			addRequest.onsuccess = function (event) {
				...
				let last_inserted_id = addRequest.result;
				localStorage["last_sale_id"] = sale_id;
				localStorage["last_sale_id_split"] = last_inserted_id;
				clearFooterCartCalculation();
				$("#hidden_given_amount").val('');
			...
			addRequest.onerror = function (event) {
				alert("Unable to add data\r\nOrder already exists in recent_sales!");
			};
		}
	}
    ...
    /* function update_online_push12(sale_id) {
		// open transaction
		...
		objectStore.openCursor().onsuccess = function (event) {
			...
			if (cursor) {
				if (cursor.value.sales_id == sale_id) {
					...
					// 🚀 Fetch fresh invoice number from server
					$.ajax({
						url: base_url + "Sale/get_invoiceNumber",
						method: "GET",
						cache: false,
						success: function (response) {
							...
							updateData.inv_no = data.inv_no;
							updateData.sale_inv_no = data.sale_inv_no;

							// ✅ NEW transaction for update
							let tx = db.transaction(['recent_sales'], "readwrite");
							let store = tx.objectStore('recent_sales');
							let req = store.put(updateData); // put() replaces existing by key

							req.onsuccess = function () {
								console.log("Updated sale with fresh invoice no:", data.inv_no);
							};
							req.onerror = function () {
								console.error("Failed to update recent_sales record");
							};
						},
						error: function () {
							console.error("Failed to fetch new invoice number for sync");
						}
					});
			...
			objectStore.openCursor().onsuccess = function (event) {
				...
				if (cursor) {
					if (cursor.value.sales_id == sale_id) {
						let updateData = cursor.value;
						updateData.online_push = 1;
						updateData.inv_no = main_inv_no;
						updateData.sale_inv_no = main_sale_inv_no;
						let request = cursor.update(updateData);
						request.onsuccess = function () {
							getNextSaleInvNo();
						}
    ...
    function push_online() {
        ...
        objectStore.openCursor().onsuccess = function (event) {
            ...
            if (cursor) {
				...
				// Lookup related record in 'sales' and delete if is_invoice === 2
                let salesTransaction = db.transaction("sales", "readwrite");
                let salesStore = salesTransaction.objectStore("sales");
                let salesRequest = salesStore.get(sales_id);
                
                salesRequest.onsuccess = function(event) {
                    let salesRecord = event.target.result;
                    if (salesRecord && salesRecord.is_invoice === 2) {
                        console.log("Deleted Sales Details: ", sales_id);
                        salesStore.delete(sales_id);
                    }
                };
    ...
    function call_print_invoice(order_info, sale_id, sale_inv_no = '', inv_no = '') {
        let order = JSON.parse(order_info);
        ...
        let subtotal_amount = 0;
            ...
            for (let key in order.items) {
                ...
                subtotal_amount += parseFloat(this_item.menu_price_without_discount);
                	// for (let mod_key in modifier_names_custom) {
                    //     let tmp_mod_name_m_n = modifier_names_custom[mod_key];
                    //     let tmp_mod_name_m_p = parseFloat(modifier_prices_custom[mod_key]);

                    //     invoice_print += `<tr>`;
                    //     invoice_print += `<td class="no-border border-bottom" style="padding-left: 38px;">` + tmp_mod_name_m_n + `</td>`;
                    //     invoice_print += `<td class="no-border border-bottom text-right">` + getAmount(tmp_mod_name_m_p) + `</td>`;
                    //     invoice_print += `</tr>`;

                    //     subtotal_amount += tmp_mod_name_m_p;
                    // }
                    for (let mod_key in modifier_names_custom) {
                        let tmp_mod_name_m_n = modifier_names_custom[mod_key];
            ...
            if (Number(order.total_discount_amount)) {
                invoice_print += `<tr>
                    ...
                    <th class="text-right">` + getAmount(order.total_discount_amount) + `</th>
                </tr>`;
            }
    ...
    function print_bill(order_info, sale_id) {
                    ...
                    <div id="wrapper">
                        <div id="receiptData">
                            <div id="receipt-data">
                                ...
                                <table style="width:100%;margin-top: 10px;">
                    <tr>
                        <td style="text-align:left"><h5 style="margin-bottom: 0px; margin-top: 0px;"><b>` + order.sale_no + `</b></h5></td>
                        <td style="text-align:right"><h5 style="margin-bottom: 0px; margin-top: 0px;"><b>`+ order_type + `</b></h5></td>
                    </tr>   
                </table>
                <table style="width:100%">
        ...
        for (let key in order.items) {
            ...
            invoice_print += `<tr>`;
            invoice_print += `<td class="no-border border-bottom ir_wid_90"><h5 style="margin-bottom: 0px;margin-top: 0px;"># ` + sl + `:` + this_item.menu_name + alternative_name;
            ...
            invoice_print += `</h5></td>`;
            invoice_print += `<td class="no-border border-bottom text-right">`;
            invoice_print += getAmount(this_item.menu_price_without_discount);
            invoice_print += `</td>`;
            invoice_print += `</tr>`;
            for (let mod_key in modifier_names_custom) {
                let tmp_mod_name_m_n = modifier_names_custom[mod_key];
    ...
    $(document).on("click", "#last_ten_sales_button", function (e) {
        ...
        objectStore.openCursor(null, 'prev').onsuccess = function (event) {
            ...
            if (cursor) {
                ...
                let rowData = JSON.parse(orderInfo);
                let sales_id = cursor.value.sales_id;
				let sale_inv_no = cursor.value.sale_inv_no;
                ...
                if (db_outlet_id == outlet_id_indexdb) {
					...
					last_10_orders +=
                        '<div class="first_column column fix">' + sale_inv_no + "</div>";
                    last_10_orders +=
    ...
    $(document).on("click", ".place_order_operation", function (e) {
        ...
        if (is_login_checker == true) {
            ...
            if (selected_order_type_object.length > 0) {
                ...
                if ($("#finalize_clicked_once").val() === "yes") {
                    // Finalized previously: generate new
                    ...
                    // Reset finalize flag after generating new order
                    $("#finalize_clicked_once").val("no");
                } else {
                    // Not finalized yet or reused same session
                    if ($("#sale_no_new_hidden").val() && $("#random_code_hidden").val()) {
                        sale_no_new = $("#sale_no_new_hidden").val();
                        random_code = $("#random_code_hidden").val();
                    } else {
                        sale_no_new = generateSaleNo();
                        random_code = getRandomCode(15);
                        $("#sale_no_new_hidden").val(sale_no_new);
        ...
        if (status == true) {
            ...
            if (sale_id > 0) {
                ...
                if (is_split_bill != 1) {
                    ...
                    order_info += '"payment_object":' + JSON.stringify(payment_info) + ',';
	...
	$.ajax({
		...
		success: function(customers) {
			$('#select2-walk_in_customer-container').empty();
			$.each(customers, function(i, customer) {
				$('#select2-walk_in_customer-container')
    ...
    function print_invoice(sale_id, type) {
        ...
        if (print_type_invoice == "web_browser_popup") {
            let res = get_all_information_of_recent_sales_from_indexeddb_print_invoice(sale_id, type).then(function (order_info) {
                call_print_invoice(order_info.order, sale_id, order_info.sale_inv_no, order_info.inv_no);
            });
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            let currentCount = parseInt($("#notification_counter").html()) || 0;

            $("#notification_counter").html(newCount);

            if (newCount > currentCount) {
                // Animate once using CSS
                $("#notification_button").addClass("flash");
                setTimeout(() => {
                    $("#notification_button").removeClass("flash");
                }, 2000);
                ...
                notifications_list += `
                    <div class="single_row_notification fix" id="single_notification_row_${notification.id}">
                        ...
                        <div class="fix single_serve_button">
                            <button class="single_serve_b" id="notification_serve_button_${notification.id}">Serve/Take/Delivery</button>
                        </div>
                    </div>`;
            });
            $("#notification_list_holder").html(notifications_list);
    ...
    // === Push Offline Orders Online ===
    push_online();

    // === Get Waiter Orders ===
    let sale_no_all = '';
    $(".running_order_order_number[data-added_offline_status='2']").each(function () {
        sale_no_all += $(this).text() + ',';
    });
    ...
    if (sale_no_all !== '') {
        $.ajax({
            ...
            success: function (response) {
                const outlet_id = $("#outlet_id_indexdb").val();
                const company_id = $("#company_id_indexdb").val();

                // Cache current running orders
                const existingSales = new Set();
                $(".running_order_order_number").each(function () {
                    existingSales.add($(this).text());
                });

                // Add new waiter orders
                (response.get_waiter_orders || []).forEach(order => {
                    let order_info = JSON.parse(order.self_order_content);
                    order_info.sale_date = getCurrentDate();
                    let sale_no = order_info.sale_no;

                    if (!existingSales.has(sale_no)) {
                        add_sale_by_ajax(JSON.stringify(order_info), 0, outlet_id, company_id, sale_no, "", "");
                ...
                // Delete orders
                (response.get_waiter_orders_for_delete_sender || []).forEach(order => {
                    deleteOrderForWaiter(order.sale_no);
                    setOrderInvoiceDeleted(order.id, 1);
                });

                (response.get_waiter_orders_for_delete_receiver || []).forEach(order => {
                    deleteOrderForWaiter(order.sale_no);
                    setOrderInvoiceDeleted(order.id, 2);
                });

                // Remove already invoiced
                (response.already_invoiced_orders || []).forEach(order => {
                    deleteOrderForWaiter(order.sale_no);
                });
            },
            error: function () {
                console.error("Failed to fetch waiter orders.");
            },
        });
    }
}
...
*/
    ...
    */
        ...
        let res = get_all_information_from_indexeddb(sale_id).then(function (data) {
            ...
            if (response !== null) {
                ...
                for (let key in response.items) {
                    ...
                    let vat_amount = isTaxEnabled === true ? get_total_vat_for_split(response.customer_id, this_item.item_vat, this_item.menu_price_with_discount) : null;
                    let total_vat_amount = 0;
                    $.each(vat_amount, function (key, value) {
                        let vat_tmp = 0;
                        if (value && value != "NaN") {
                    ...
                    for (let mod_key in modifier_names_custom) {
                        let tmp_mod_name_m_n = modifier_names_custom[mod_key];
                        let tmp_mod_name_m_p = Number(modifier_prices_custom[mod_key]).toFixed(ir_precision);
                        let split_vat_amount = isTaxEnabled === true ? get_total_vat_for_split(response.customer_id, JSON.parse(modifier_vats[mod_key]), (Number(tmp_mod_name_m_p) * Number(this_item.qty))) : null;
    ...
    function cal_split_modal() {
        ...
        let vat_amount_tmp = isTaxEnabled === true ? get_total_vat_for_split_sale_table() : null;
