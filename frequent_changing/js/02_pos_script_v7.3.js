(function ($) {
                ...
                for (let k in tax_information) {
                    if (tax_name.includes(tax_information[k].tax_field_name) && checkTaxApply(tax_information[k].tax_field_name)) {
                        ...
                        if (checkTaxApply(tax_information[k].tax_field_name)) {
                            ...
                            if (tax_type == 1) {
                                current_value = (parseFloat(tax_information[k].tax_field_percentage) * parseFloat(item_total_price)) / 100;
                            } else {
                                let taxAmount = totalTaxPercentage > 0 ? (parseFloat(item_total_price) * totalTaxPercentage) / (100 + totalTaxPercentage) : 0;
                                current_value = taxAmount / 2;
                            }

                            // assign safely
                            tax_object[tax_information[k].tax_field_name] = current_value.toFixed(ir_precision);
    ...
    function set_finalize_discount() {
        ...
        //sub_total_discount_finalize = Number($("#sub_total_discount_finalize").val()) || 0;

        let finalize_total_paid = Number($("#finalize_total_paid").text());
    ...
    $(document).on("click", "#finalize_order_button", function (e) {
        ...
        if (is_login_checker == true) {
            ...
            if (selected_order_type_object.length > 0) {
                ...
                console.log("FINAL ORDER INFO:", order_info);
    ...
    // Reset hidden fields
    $("#update_sale_id").val('');
    $("#sale_no_new_hidden").val('');
    $("#random_code_hidden").val('');
    $("#finalize_clicked_once").val("no");
    ...
    $("#finalize_total_due").html("0.00");

    // Reset discount & tips
    $("#sub_total_discount").val('');
    $("#tips_amount").val('');
    $("#show_tips_amount").html("0.00");

    // Reset charges
    $("#delivery_charge").val('');
    $("#show_charge_amount").html("0.00");

    // Reset customer & waiter
    $("#walk_in_customer").val("1").trigger("change"); // walk-in
    $("#select_waiter").val("").trigger("change");
    ...
    function add_sale_by_ajax(order_info) {
        $.ajax({
            ...
            data: {
                orders: order_info,
                sales_id: '', // If needed, pass actual sale_id here
    ...
    function call_print_invoice(order_info, sale_inv_no = '', inv_no = '') {
      ...
      let sale_date_split = [];
        if (order.date_time && typeof order.date_time === "string") {
            // already contains both date & time
            sale_date_split = order.date_time.split(" ");
        } else if (order.sale_date || order.order_time) {
            // fallback: take sale_date and order_time separately
            let s_date = order.sale_date ? order.sale_date : "-";
            let s_time = order.order_time ? order.order_time : "-";
            sale_date_split = [s_date, s_time];
        } else {
            console.warn("Missing date_time, sale_date and order_time in order:", order);
            sale_date_split = ["-", "-"];  // show dash if nothing
        }
        ...
        //         }
        //         invoice_print += `<tr>
        //             <th class="text_left">Paid (${value.payment_name}${txt_point})</th>
        //             <th class="text-right">${getAmount(value.amount)}</th>
        //         </tr>`;
                
        //         total_paid += parseFloat(value.amount) || 0; // accumulate
        //     });

        //     // Add final total row
        //     invoice_print += `<tr>
        //         <th class="text_left">Total Amount</th>
        //         <th class="text-right">${getAmount(total_paid)}</th>
        //     </tr>`;
        // }
    ...
    let placeOrderSound = new Howl({
        ...
        $.ajax({
            url: base_url + "Sale/getLastTenSales",   // ✅ your CodeIgniter route
            method: "GET",
            dataType: "json",
            success: function (response) {
                let last_10_orders = "";
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            //     // notes, combo, modifiers etc can be appended same way as before...
            //     draw_table_for_last_ten_sales_order += "</div>";
            // }
...
for (let key in response.items) {
    ...
    $(document).on("click", "#print_last_invoice", function (e) {
        ...
        // Fetch last 10 sales
        $.ajax({
            url: base_url + "Sale/getLastTenSales",  // change to your actual URL
            type: 'GET',
            dataType: 'json',
            success: function(sales) {
                if (sales && sales.length > 0) {
                    ...
                    // Optionally, store it in localStorage if you need
                    localStorage['last_sale_id'] = sale_id;

                    // Print invoice
                    print_invoiceResent(sale_id, 1);
                } else {
                    toastr['error'](txt_err_pos_4, '');
                }
            },
           error: function(xhr, status, error) {
        console.error("AJAX Error:", status, error);
        console.log(xhr.responseText); // <-- shows server output
    ...
    //                     // Build the same structure as your IndexedDB version
    //                     let orderInfo = {
    //                         order: response.order,          // your order array/object
    //                         sale_inv_no: response.sale_inv_no,
    //                         inv_no: response.inv_no
    //                     };
    //                     resolve(orderInfo);
    //                 } else {
    //                     reject("Sale not found in database");
    //                 }
    //             },
    //             error: function(xhr, status, error) {
    //                 reject(error);
