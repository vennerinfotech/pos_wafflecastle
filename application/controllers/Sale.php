...
class Sale extends Cl_Controller
{
    ...
    {
        log_message('debug', 'POS() STARTED with params: user_id=' . $user_id . ', outlet_id=' . $outlet_id . ', sale_id=' . $sale_id);
        $is_waiter = $this->session->userdata('is_waiter');
        ...
        if ($is_self_order == 'Yes' && !$outlet_id) {
            echo 'Something is wrong, please scan QR-Code again';
            exit;
        }
        if ($counter_id) {
    ...
    public function Save()
    {
        ...
        // ////////////////////////////////
        // Optimize: Prepare batch data and fetch ingredients once
        $batch_sales_details = [];
        $batch_consumptions_menus = [];
        $user_id = $this->session->userdata('user_id');

        // Fetch all ingredients for these menu items in one query
        $unique_menu_ids = array_unique($food_menu_id);
        $all_ingredients_flat = $this->Sale_model->getIngredientsForMultipleMenus($unique_menu_ids);

        // Group ingredients by food_menu_id for easy lookup
        $ingredients_by_menu = [];
        if ($all_ingredients_flat) {
            foreach ($all_ingredients_flat as $ing) {
                $ingredients_by_menu[$ing->food_menu_id][] = $ing;
            }
        }
        ...
        foreach ($food_menu_id as $value) {
            ...
            // Prepare Consumption Details
            if (isset($ingredients_by_menu[$value])) {
                foreach ($ingredients_by_menu[$value] as $inrow) {
                    $batch_consumptions_menus[] = array(
                        'sale_consumption_id' => $sale_consumption_id,
                        'ingredient_id' => $inrow->ingredient_id,
                        'consumption' => $inrow->consumption * $qty[$i],
                        'user_id' => $user_id,
                        'outlet_id' => $outlet_id
                    );
                }
        ...
        // Batch Insert
        if (!empty($batch_sales_details)) {
            $this->db->insert_batch('tbl_sales_details', $batch_sales_details);
        }
        if (!empty($batch_consumptions_menus)) {
            $this->db->insert_batch('tbl_sale_consumptions_of_menus', $batch_consumptions_menus);
        }
    ...
    public function add_customer_by_ajax()
    {
        ...
        // 1️⃣ Check if phone already exists in Live customers
        ...
        // 2️⃣ Build data for new customer
        $data['name'] = trim_checker(htmlspecialcharscustom($this->input->post($this->security->xss_clean('customer_name'))));
    ...
    //     // Optional: pass company_id if needed for filtering
    //     $company_id = $this->session->userdata('company_id');

    //     $data = $this->Comman_model->getCustomersAjax($company_id);
    ...
    // public function ajax_get_customers() {
    //     $this->load->model("Comman_model");
    //     $data = $this->Comman_model->getCustomersAjax();
    //     echo json_encode($data);
    // }
    ...
    public function ajax_get_customers()
    {
        ...
        $company_id = $this->session->userdata('company_id');
        $user_id = $this->session->userdata('user_id');  // outlet from session

        $this->db->select('id, name, phone');
        $this->db->from('tbl_customers');
        $this->db->where('company_id', $company_id);
        $this->db->where('user_id', $user_id);  // filter by outlet_id also
    ...
    //     public function getLastTenSales() {
    //     $outlet_id = $this->session->userdata('outlet_id');
    //     $company_id = $this->session->userdata('company_id');

    //     $this->db->select('id as sales_id, sale_no, sale_inv_no, customer_name, phone, table_name, orders_table_text, token_number');
    ...
    //     $this->db->select('id as sales_id, sale_no, sale_inv_no, outlet_id, customer_id');
    //     $this->db->from('tbl_sales');
    //     $this->db->where('outlet_id', $outlet_id);
    //     $this->db->where('company_id', $company_id);
    //     $this->db->order_by('id', 'DESC');
    ...
    //     header('Content-Type: application/json'); // ✅ proper header
    //     echo json_encode($sales);
    // }
    ...
    //         $this->db->from('tbl_sales s');
    //         $this->db->join('tbl_customers c','s.customer_id = c.id','left');
    //         $this->db->join('tbl_users w','s.waiter_id=w.id','left');
    //         $this->db->join('tbl_tables t','s.table_id=t.id','left');
    ...
    //             // Attach modifiers with names to each item
    //             foreach ($items as &$item) {
    //                 $item['modifiers'] = $this->db
    //                     ->select('m.id as modifier_id, m.name as modifier_name, sdm.modifier_price')
    //                     ->from('tbl_sales_details_modifiers sdm')
    //                     ->join('tbl_modifiers m', 'sdm.modifier_id = m.id', 'left')
    //                     ->where('sdm.sales_details_id', $item['id'])
    //                     ->get()
    //                     ->result_array();
    //             }
    ...
    public function getAllPayments()
    {
        return $this
            ->db
            ->select('id, name, description, user_id, company_id, order_by, personalinformation, del_status')
            ->from('tbl_payment_methods')
            ->where('del_status', 'Live')  // only active
            ->order_by('order_by', 'ASC')
            ->get()
            ->result_array();  // return as array
    }
...
\t\t\ts.payment_method_id, 
        ...
        ");
            ...
            ')
                ...
                $sale['items'] = $items;
                $sale['sale_inv_no'] = $sale['sale_inv_no'] ?? null;
                $sale['inv_no'] = $sale['sale_no'] ?? null;
    ...
    public function get_last_10_sales()
    {
        $company_id = $this->session->userdata('company_id');
        $outlet_id = $this->session->userdata('outlet_id');
        ...
        s.sub_total,
        s.sub_total_with_discount,
        s.sub_total_discount_value,
        s.sub_total_discount_amount,
        s.total_discount_amount,
        s.total_item_discount_amount,
        s.delivery_charge,
        s.tips_amount,
        s.total_payable,
        c.id as customer_id,
    ...
    ');
        ...
        $this->db->join('tbl_users w', 'w.id = s.waiter_id', 'left');
    ...
    //         if ($sale) {
    //             // 🔧 change 'sales_id' to your actual foreign key column
    //             $sale['items'] = $this->db->select('*')
    //                                       ->from('tbl_sales_details')
    //                                       ->where('sales_id', $sale_id)
    //                                       ->get()->result_array();
    ...
    //         $this->db->from('tbl_sales s');
    //         $this->db->join('tbl_customers c','s.customer_id = c.id','left');
    //         $this->db->join('tbl_users w','s.waiter_id=w.id','left'); // waiter details
    //         $this->db->join('tbl_tables t','s.table_id=t.id','left'); // table details
    //         $this->db->where('s.id', $sale_id);
    //         $this->db->where('s.outlet_id', $outlet_id);
    //         $this->db->where('s.company_id', $company_id);
    ...
    // {
    //     $order = json_decode($this->input->post('orders'));
    //     if (!$order) {
    ...
    //     $details = !empty($order->payment_object) ? json_decode($order->payment_object) : [];
    //     $firstPay = is_array($details) && !empty($details) ? $details[0] : null;
    //     $payment_id   = $firstPay->payment_id   ?? ($order->payment_method_type ?? null);

    //     // Base sale data
    //     $data = [
    //         'self_order_content'          => $this->input->post('orders'),
    //         'customer_id'                 => $order->customer_id ?? 1,
    //         'counter_id'                  => $order->counter_id ?? null,
    //         'delivery_partner_id'         => $order->delivery_partner_id ?? null,
    //         'total_items'                 => $order->total_items_in_cart ?? 0,
    //         'sub_total'                   => $order->sub_total ?? 0,
    //         'charge_type'                 => $order->charge_type ?? '',
    //         'previous_due_tmp'            => $order->previous_due_tmp ?? 0,
    //         'vat'                         => 0,
    //         'total_payable'               => $order->total_payable ?? 0,
    //         'total_item_discount_amount'  => $order->total_item_discount_amount ?? 0,
    //         'sub_total_with_discount'     => $order->sub_total_with_discount ?? 0,
    //         'sub_total_discount_amount'   => $order->sub_total_discount_amount ?? 0,
    //         'total_discount_amount'       => $order->total_discount_amount ?? 0,
    //         'tips_amount'                 => $order->tips_amount ?? 0,
    //         'tips_amount_actual_charge'   => $order->tips_amount_actual_charge ?? 0,
    //         'delivery_charge'             => $order->delivery_charge ?? 0,
    //         'delivery_charge_actual_charge'=> $order->delivery_charge_actual_charge ?? 0,
    //         'sub_total_discount_value'    => $order->sub_total_discount_value ?? 0,
    ...
    //         'outlet_id'                   => $this->session->userdata('outlet_id'),
    //         'company_id'                  => $this->session->userdata('company_id'),
    //         'sale_date'                   => date('Y-m-d', strtotime($order->date_time)),
    //         'date_time'                   => date('Y-m-d H:i:s', strtotime($order->date_time)),
    //         'order_time'                  => date('Y-m-d H:i:s', strtotime($order->date_time)),
    //         'paid_date_time'              => date('Y-m-d H:i:s'),
    //         'order_status'                => 3,
    //         'orders_table_text'           => $order->orders_table_text ?? '',
    ...
    //     // VAT recalculation
    //     $total_tax = 0;
    //     if (!empty($order->sale_vat_objects)) {
    //         foreach ($order->sale_vat_objects as $vat) {
    //             $total_tax += $vat->tax_field_amount ?? 0;
    //         }
    //     }
    //     $data['vat'] = $total_tax;
    //     $data['sale_vat_objects'] = json_encode($order->sale_vat_objects ?? []);
    ...
    //         $this->db->delete('tbl_sales_details', ['sales_id' => $sale_id]);
    //         $this->db->delete('tbl_sales_details_modifiers', ['sales_id' => $sale_id]);
    //         $this->db->delete('tbl_sale_consumptions', ['sale_id' => $sale_id]);
    //         $this->db->delete('tbl_sale_consumptions_of_menus', ['sales_id' => $sale_id]);
    //         $this->db->delete('tbl_sale_consumptions_of_modifiers_of_menus', ['sales_id' => $sale_id]);
    //         $this->db->delete('tbl_sale_payments', ['sale_id' => $sale_id]);
    ...
    //         $this->db->where('id', $sales_id)->update('tbl_sales', [
    //             'sale_no'      => $sale_no,
    //             'inv_no'       => $inv_no,
    ...
    //     // Order tables
    //     if (!empty($order->orders_table)) {
    //         foreach ($order->orders_table as $ot) {
    //             $this->db->insert('tbl_orders_table', [
    //                 'persons'     => $ot->persons ?? 0,
    //                 'booking_time'=> date('Y-m-d H:i:s'),
    //                 'sale_id'     => $sales_id,
    //                 'sale_no'     => $sale_no,
    //                 'outlet_id'   => $this->session->userdata('outlet_id'),
    //                 'table_id'    => $ot->table_id ?? null,
    //             ]);
    //         }
    //     }

    //     // Consumption record
    //     $this->db->insert('tbl_sale_consumptions', [
    //         'sale_id'   => $sales_id,
    //         'user_id'   => $this->session->userdata('user_id'),
    //         'outlet_id' => $this->session->userdata('outlet_id'),
    //         'del_status'=> 'Live',
    ...
    //     // Items + modifiers + consumptions
    //     if (!empty($order->items)) {
    //         foreach ($order->items as $item) {
    //             $food = $this->Common_model->getDataById($item->food_menu_id, "tbl_food_menus");
    ...
    //                 'qty'                         => $item->qty ?? 0,
    //                 'tmp_qty'                     => $item->qty ?? 0,
    //                 'menu_price_without_discount' => $item->menu_price_without_discount ?? 0,
    //                 'menu_price_with_discount'    => $item->menu_price_with_discount ?? 0,
    //                 'menu_combo_items'            => $item->menu_combo_items ?? '',
    //                 'is_free_item'                => $item->is_free ?? 0,
    //                 'menu_unit_price'             => $item->menu_unit_price ?? 0,
    //                 'menu_taxes'                  => json_encode($item->item_vat ?? []),
    //                 'menu_discount_value'         => $item->menu_discount_value ?? 0,
    //                 'discount_type'               => $item->discount_type ?? '',
    //                 'menu_note'                   => $item->item_note ?? '',
    //                 'discount_amount'             => $item->item_discount_amount ?? 0,
    //                 'item_type'                   => ($this->Sale_model->getItemType($item->food_menu_id)->item_type == "Bar No") ? "Kitchen Item" : "Bar Item",
    //                 'cooking_status'              => $item->item_cooking_status ?: null,
    //                 'cooking_start_time'          => (!empty($item->item_cooking_start_time) && $item->item_cooking_start_time != "0000-00-00 00:00:00") ? date('Y-m-d H:i:s', strtotime($item->item_cooking_start_time)) : '0000-00-00 00:00:00',
    //                 'cooking_done_time'           => (!empty($item->item_cooking_done_time) && $item->item_cooking_done_time != "0000-00-00 00:00:00") ? date('Y-m-d H:i:s', strtotime($item->item_cooking_done_time)) : '0000-00-00 00:00:00',
    //                 'previous_id'                 => $item->item_previous_id ?: 0,
    //                 'sales_id'                    => $sales_id,
    //                 'user_id'                     => $this->session->userdata('user_id'),
    //                 'outlet_id'                   => $this->session->userdata('outlet_id'),
    //                 'loyalty_point_earn'          => ($order->customer_id != 1) ? ($item->qty * getLoyaltyPointByFoodMenu($item->food_menu_id, '')) : 0,
    //                 'del_status'                  => 'Live',
    //             ];
    ...
    //             if (empty($item->item_previous_id)) {
    //                 $this->db->where('id', $sales_details_id)->update('tbl_sales_details', ['previous_id' => $sales_details_id]);
    //             }

    //             // Ingredient consumption
    //             if ($food && $food->product_type == 1) {
    //                 $ingredients = $this->db->query("SELECT * FROM tbl_food_menus_ingredients WHERE food_menu_id={$item->food_menu_id}")->result();
    //                 foreach ($ingredients as $ing) {
    //                     $this->db->insert('tbl_sale_consumptions_of_menus', [
    //                         'ingredient_id'       => $ing->ingredient_id,
    //                         'consumption'         => $item->qty * $ing->consumption,
    //                         'sale_consumption_id' => $sale_consumption_id,
    //                         'sales_id'            => $sales_id,
    //                         'cost'                => $ing->cost,
    //                         'food_menu_id'        => $item->food_menu_id,
    ...
    //             // Modifiers
    //             if (!empty($item->modifiers_id)) {
    //                 $modifier_ids = explode(",", $item->modifiers_id);
    //                 $modifier_prices = !empty($item->modifiers_price) ? explode(",", $item->modifiers_price) : [];
    //                 $modifier_vats = !empty($item->modifier_vat) ? explode("|||", $item->modifier_vat) : [];

    //                 foreach ($modifier_ids as $i => $mid) {
    //                     $this->db->insert('tbl_sales_details_modifiers', [
    //                         'modifier_id'       => $mid,
    //                         'modifier_price'    => $modifier_prices[$i] ?? 0,
    //                         'food_menu_id'      => $item->food_menu_id,
    //                         'sales_id'          => $sales_id,
    //                         'sales_details_id'  => $sales_details_id,
    //                         'menu_taxes'        => $modifier_vats[$i] ?? '',
    //                         'user_id'           => $this->session->userdata('user_id'),
    //                         'outlet_id'         => $this->session->userdata('outlet_id'),
    ...
    //     // Payments
    //     if (!empty($order->payment_object)) {
    //         $payment_details = json_decode($order->payment_object);
    ...
    //                 'outlet_id'   => $this->session->userdata('outlet_id'),
    //             ];
    //             $this->Common_model->insertInformation($payData, "tbl_sale_payments");
    //         }
    //     }
    ...
    //     // SMS
    //     if (!empty($order->send_sms_status)) {
    //         $customer = getCustomerData($order->customer_id ?? 1);
    //         $outlet_name = $this->session->userdata('outlet_name');
    //         $sms = "Hi {$customer->name}, thank you for visiting {$outlet_name}. "
    //             . "Your bill on {$order->date_time} is " . getAmtCustom($order->total_payable)
    //             . ". Paid: " . getAmtCustom($order->paid_amount)
    //             . ", Due: " . getAmtCustom($order->due_amount) . ".";
    //         if (!empty($customer->phone)) {
    //             smsSendOnly($sms, $customer->phone);
    //         }
    //     }
    ...
    public function push_online()
    {
        ...
        if (!empty($order->payment_object)) {
            ...
            foreach ($payment_details as $p) {
                ...
                $payData = [
                    'payment_id' => $p->payment_id,  // Payment method ID
                    'payment_name' => $p->payment_name,  // Payment name (e.g., "Credit Card")
                    'amount' => $p->amount,  // Payment amount
                    'date_time' => date('Y-m-d H:i:s', strtotime($order->date_time)),  // Payment date and time
                    'sale_id' => $sale_id,  // Associate with the sale ID
                    'counter_id' => $this->session->userdata('counter_id'),  // Counter ID
                    'user_id' => $this->session->userdata('user_id'),  // User ID (who processed the payment)
                    'outlet_id' => $this->session->userdata('outlet_id'),  // Outlet ID
                ];

                // Insert payment information into tbl_sale_payments
                $this->Common_model->insertInformation($payData, 'tbl_sale_payments');
            }

            $latest_payment = end($payment_details);  // Get the last inserted payment object
            if (isset($latest_payment->payment_id)) {
                // Update tbl_sales with the payment_id
                $this->db->where('id', $sale_id)->update('tbl_sales', [
                    'payment_method_id' => $latest_payment->payment_id  // Update with the last payment method
                ]);
            }
        }
...
\t\t\t " . $html . '
    ...
    // {
    //     $outlet_id = $this->session->userdata('outlet_id'); // outlet-wise filtering

    //     $this->db->select('sale_id');
    //     $this->db->from('tbl_sales');
    //     $this->db->where('outlet_id', $outlet_id); // only current outlet
    ...
    //     if ($query->num_rows() > 0) {
    //         $result = $query->row();
    //         echo json_encode(['sale_id' => $result->sale_id]);
    //     } else {
    //         echo json_encode(['sale_id' => null]);
    //     }
    // }
}
