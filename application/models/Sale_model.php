...
class Sale_model extends CI_Model
{
  ...
  /**
   * get Food Menu Ingredients for multiple menus
   * @access public
   * @return object
   * @param array
   */
  public function getIngredientsForMultipleMenus($food_menu_ids)
  {
    ...
    $this->db->select('*');
    $this->db->from('tbl_food_menus_ingredients');
    $this->db->where_in('food_menu_id', $food_menu_ids);
    $this->db->where('del_status', 'Live');
    $result = $this->db->get();
  ...
  public function is_tax_collection_enabled()
  {
    $outlet_id = $this->session->userdata('outlet_id');
    ...
    $this->db->select('collect_tax');
    $this->db->from('tbl_outlets');
    $this->db->where('id', $outlet_id);
    $query = $this->db->get();
    $result = $query->row();

    return ($result && !empty($result->collect_tax) && $result->collect_tax !== 'No');
  }
  ...
  public function getAllFoodMenus()
  {
    ...
    // Optimized Query: Removed expensive JOIN on tbl_sales_details and GROUP BY which caused temp tables
    $result = $this->db->query("SELECT fm.*, fmc.category_name, 0 as item_sold, kt_cat.kitchen_id
        FROM tbl_food_menus fm
        LEFT JOIN tbl_food_menu_categories fmc ON fmc.id = fm.category_id AND fmc.del_status='Live'
        LEFT JOIN tbl_kitchen_categories kt_cat ON kt_cat.cat_id = fm.category_id AND kt_cat.del_status='Live'
        WHERE FIND_IN_SET(fm.id, '$getFM') AND fm.del_status='Live'
        GROUP BY fm.id
        ORDER BY fm.name ASC")->result();
  ...
  public function getWaitersForThisCompany($company_id, $table)
  {
    ...
    if (str_rot13($language_manifesto) == 'eriutoeri') {
      ...
      $this->db->where("FIND_IN_SET('$value', outlets) >", 0, false);
  ...
  // 		// 🔒 Start transaction (avoid duplicates if two cashiers at same time)
  // 		$this->db->trans_start();
  ...
  // 		$this->db->where('company_id', $company_id);
  // 		$this->db->where('outlet_id', $outlet_id);
  // 		$this->db->where('sale_date >=', $financialYearStart);
  // 		$this->db->where('sale_date <=', $financialYearEnd);
  // 		$query = $this->db->get();

  // 		$lastInvNo = $query->row()->inv_no ?? 0;
  // 		$newInvNo  = $lastInvNo + 1;

  // 		// Format like 00001, 00002
  // 		$newInvNoFormatted = str_pad($newInvNo, 5, '0', STR_PAD_LEFT);

  // 		$this->db->trans_complete();

  // 		if ($this->db->trans_status() === FALSE) {
  // 			throw new Exception("DB transaction failed when generating invoice.");
  // 		}

  // 		return $newInvNoFormatted;

  // 	} catch (Exception $e) {
  // 		log_message('error', 'Error in getNextInvoiceNumber: ' . $e->getMessage());
  // 		return "00001";
  // 	}
  // 	}
  ...
  /*  public function getNextInvoiceNumber() {
      try {
          ...
          $sql = "
              SELECT MAX(inv_no) as lastInvNo
              FROM tbl_sales
              WHERE company_id = ?
                AND outlet_id = ?
                AND sale_date >= ?
          ...
          $query = $this->db->query($sql, [$company_id, $outlet_id, $financialYearStart, $financialYearEnd]);
          $lastInvNo = $query->row()->lastInvNo ?? 0;
  ...
  public function getNextInvoiceNumber()
  {
    try {
      ...
      $query = $this
        ...
        ->where('sale_date >=', $financialYearStart)
        ->where('sale_date <=', $financialYearEnd)
        ->get();
      ...
      custom_log('error', 'Error in getNextInvoiceNumber: ' . $e->getMessage());
      return 1;
    }
  }
}
